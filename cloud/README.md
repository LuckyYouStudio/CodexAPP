...
1. **真账号系统**：现在是最简 email/password + 内存 token（dev 自动注册）。生产要：数据库、JWT/刷新令牌、邮箱验证、找回密码。
...
5. **iOS/Android App**：✅ 云模式已接——`mobile/`（Expo）现支持「云账号」模式：账号登录 +
   端到端加密（`mobile/src/e2e.js`，与 Agent 互通已测）。**仍待**：APNs 推送、App 内购买(IAP)、上架。
